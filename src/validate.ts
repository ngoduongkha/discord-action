...
export function fitEmbed(embed: any): any {
  ...
  if (embed.description) {
    ...
    if (descLen > constants.MAX_EMBED_DESCRIPTION_LENGTH) {
      ...
      embed.description = truncStr(
        embed.description,
        constants.MAX_EMBED_DESCRIPTION_LENGTH
      );
    }
  }
  if (embed.fields) {
    for (const field of embed.fields) {
      ...
      if (nameLen > constants.MAX_EMBED_FIELD_NAME_LENGTH) {
        ...
        field.name = truncStr(
          field.name,
          constants.MAX_EMBED_FIELD_NAME_LENGTH
        );
      }
      if (valueLen > constants.MAX_EMBED_FIELD_VALUE_LENGTH) {
        ...
        field.value = truncStr(
          field.value,
          constants.MAX_EMBED_FIELD_VALUE_LENGTH
        );
      }
    }
  }
